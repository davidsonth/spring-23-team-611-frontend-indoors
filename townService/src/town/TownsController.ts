...
import DanceAreaReal from './DanceArea';
...
export class TownsController extends Controller {
   ...
   *
   * @throws InvalidParametersError if the session token is not valid, or if the
   *         dance area could not be created
   */
  @Post('{townID}/danceArea')
  @Response<InvalidParametersError>(400, 'Invalid values specified')
  public async createDanceArea(
    ...
    // add dance area to the town, throw error if it fails
    if (!curTown.addDanceArea(requestBody)) {
      throw new InvalidParametersError('Invalid dance area');
    }
  }

  @Patch('{townID}/danceAreas/{danceAreaID}/userclick')
  @Response<InvalidParametersError>(400, 'Invalid values specified')
  public async recordDanceClick(
    @Path() townID: string,
    @Header('X-Session-Token') sessionToken: string,
    @Path() danceAreaID: string,
    @Body() requestBody: Arrow,
  ): Promise<void> {
    const curTown = this._townsStore.getTownByID(townID);
    if (!curTown) {
      throw new InvalidParametersError('Invalid town ID');
    ...
    if (!danceArea || isDanceArea(danceArea)) {
      throw new InvalidParametersError('Invalid DanceArea ID');
    }
    const currentModel: DanceArea = (<DanceAreaReal>danceArea).toModel();
    const newUserClicks = currentModel.userClicks;
    newUserClicks.push(requestBody);
    const newModel: DanceArea = { ...currentModel, userClicks: newUserClicks };
    (<DanceAreaReal>danceArea).updateModel(newModel);
  }
