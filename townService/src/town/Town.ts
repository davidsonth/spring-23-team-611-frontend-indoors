...
import { isDanceArea, isPosterSessionArea, isViewingArea } from '../TestUtils';
import {
  ChatMessage,
...
export default class Town {
    ...
    socket.on('interactableUpdate', (update: Interactable) => {
      if (isViewingArea(update)) {
        ...
        const danceArea = this._interactables.find(
          currInteractable => currInteractable.id === update.id,
        );
        if (danceArea) {
          (danceArea as DanceArea).updateModel(update);
        }
      }
    });
    return newPlayer;
  ...
  public addDanceArea(danceArea: DanceAreaModel): boolean {
    const existingDanceArea = <DanceArea>this._interactables.find(area => area.id === danceArea.id);

    if (!existingDanceArea) {
      return false;
    }
    existingDanceArea.updateModel(danceArea);
    existingDanceArea.addPlayersWithinBounds(this._players);
    this._broadcastEmitter.emit('interactableUpdate', existingDanceArea.toModel());
    return true;
  }
  ...
  public initializeFromMap(map: ITiledMap) {
    ...
    const danceAreas = objectLayer.objects
      ...
      .map(eacnDanceAreaObj => DanceArea.fromMapObject(eacnDanceAreaObj, this._broadcastEmitter));
