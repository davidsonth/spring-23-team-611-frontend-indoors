...
describe('Town', () => {
  ...
  describe('[OMG4 initializeFromMap]', () => {
    ...
    it('Creates a DanceArea instance for each region on the map', async () => {
      town.initializeFromMap(testingMaps.twoDance);
      const danceArea1 = town.getInteractable('Name1');
      const danceArea2 = town.getInteractable('Name2');
      expect(danceArea1.id).toEqual('Name1');
      expect(danceArea1.boundingBox).toEqual({ x: 40, y: 120, height: 237, width: 326 });
      expect(danceArea2.id).toEqual('Name2');
      expect(danceArea2.boundingBox).toEqual({ x: 612, y: 120, height: 266, width: 467 });
      expect(town.interactables.length).toBe(2);
    });
